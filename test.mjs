...
const emptyCreated = await request("tools/call", { name: "create_directory", arguments: { path: "empty-directory", recursive: false } });
assert.equal(emptyCreated.isError, undefined);
const emptyDeleted = await request("tools/call", { name: "delete_directory", arguments: { path: "empty-directory" } });
assert.equal(emptyDeleted.isError, undefined);
await assert.rejects(access(path.join(root, "empty-directory")));
